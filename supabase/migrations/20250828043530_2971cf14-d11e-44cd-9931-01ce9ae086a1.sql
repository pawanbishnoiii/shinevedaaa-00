-- ShineVeda B2B Export Platform Database Schema - Fixed
...
-- Inquiry counter for generating sequential numbers
CREATE TABLE public.inquiry_counter (
...
INSERT INTO public.inquiry_counter (id, current_count, year, month) VALUES (1, 0, EXTRACT(YEAR FROM NOW()), EXTRACT(MONTH FROM NOW()));
...
ALTER TABLE public.inquiry_counter ENABLE ROW LEVEL SECURITY;
...
DECLARE
    ...
    inquiry_num TEXT;
BEGIN
    -- Get or update counter
    SELECT * INTO counter_row FROM public.inquiry_counter WHERE id = 1 FOR UPDATE;
    
    -- Reset counter if new year or month
    IF counter_row.year != current_year OR counter_row.month != current_month THEN
        new_count := 1;
        UPDATE public.inquiry_counter 
        SET current_count = new_count, year = current_year, month = current_month 
        WHERE id = 1;
    ELSE
        new_count := counter_row.current_count + 1;
        UPDATE public.inquiry_counter 
    ...
    inquiry_num := 'INQ-' || current_year || '-' || LPAD(current_month::TEXT, 2, '0') || '-' || LPAD(new_count::TEXT, 4, '0');
...
-- RLS Policies for Inquiry Counter
CREATE POLICY "Admin can manage inquiry counter" ON public.inquiry_counter
    FOR ALL USING (public.is_admin());
...
CREATE TRIGGER inquiries_auto_number BEFORE INSERT ON public.inquiries FOR EACH ROW EXECUTE FUNCTION public.handle_new_inquiry();
